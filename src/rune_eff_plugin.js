...
//<img src="../assets/runes/${this.rune.sets[rune.set_id]}.png" />
  ...
  mountGemValuesTable(rune,config) {
	  ...
	  rune.sec_eff.forEach(stat=> {
		  for (var sub in this.geteffectTypeStrings) {
			  if (sub >0) {
				let val =this.getSubStatGemEfficiency(rune,stat[0],sub,4,config);
	  ...
	  let lastItem = effArr.length-1<3 ? effArr.length-1 :3;
	  let vals = effArr.slice(0,lastItem);
	  let limit = vals[lastItem-1];
	  //add substat info
	  rune.sec_eff.forEach(stat => {
		...
		for (var sub in this.geteffectTypeStrings) {
			if (sub >0) {
			  let newEfficiency = this.getSubStatGemEfficiency(rune,stat[0],sub,4,config);
				let colorStyle = parseFloat(newEfficiency.maxGrinded)>=limit ? 'bgcolor="lightgreen"' : '';
	  ...
	  rune.sec_eff.forEach(stat=> {
		  for (var sub in this.geteffectTypeStrings) {
			  if (sub >0) {
				let val =this.getSubStatGemEfficiency(rune,stat[0],sub,5,config);
				effVal.eff = val.maxGrinded=='' ? 0 : val.maxGrinded;
				effArr.push(effVal.eff);
			  }
		  }
	  ...
	  rune.sec_eff.forEach(stat => {
		...
		for (var sub in this.geteffectTypeStrings) {
			if (sub >0) {
				let newEfficiency = this.getSubStatGemEfficiency(rune,stat[0],sub,5,config);
				let colorStyle = parseFloat(newEfficiency.maxGrinded)>=limit2 ? 'bgcolor="lightgreen"' : '';
				html = html.concat(`<td ${colorStyle}>${newEfficiency.maxGrinded}</td>`);
			}
		}
		html=html.concat(`</tr>`);
 ...
 // sub stats
    rune.sec_eff.forEach(stat => {
      let value = stat[3] && stat[3] > 0 ? stat[1] + stat[3] : stat[1];
	  let maxValue = (config.flatStatsHalf ? this.rune.substat[stat[0]].max50[6] : this.rune.substat[stat[0]].max[6]);
      ratio += value / maxValue;

	  let grindValue = this.grindstone[stat[0]].range[4].max;
    ...
    // innate stat
    if (rune.prefix_eff && rune.prefix_eff[0] > 0) {
		let maxValue = (config.flatStatsHalf ? this.rune.substat[rune.prefix_eff[0]].max50[6] : this.rune.substat[rune.prefix_eff[0]].max[6]);
  ...
  getSubStatGemEfficiency(rune,statOrig, statReplace,grindLevel,config, toFixed = 2) {
    ...
    let maxValue = 0
		if (statOrig == stat[0]){
			...
			let grindValue = this.grindstone[statReplace].range[grindLevel].max;
		  ...
		  value = stat[3] && stat[3] > 0 ? stat[1] + stat[3] : stat[1];
			let grindValue = this.grindstone[stat[0]].range[grindLevel].max;
			value = value > stat[1] + grindValue ? value : stat[1] + grindValue;
